...
void do_push(int n)
{
  if (vm.sp == 255)
  {
    fprintf(stderr, "Cannot fill up stack.");
    exit(1);
  }
  vm.stack[vm.sp] = n;
  vm.sp++; 
...
void do_load(int n)
{
  if (n < 0 || n > 16)
  {
    fprintf(stderr, "Cannot load from a non-existent register.");
    exit(1);
  }  
  do_push(vm.reg[n]);
}
...
void do_store(int n)
{
  if (n < 0 || n > 16)
  {
    fprintf(stderr, "Cannot store into a non-existent register.");
    exit(1);
  }
  if (vm.sp == 0)
  {
    fprintf(stderr, "Cannot store when stack is empty.");
    exit(1);
  }  
...
void do_jmp(int n)
{
  if (vm.inst[n] == 0)
  {
    fprintf(stderr, "Cannot read beyond the end of program instruction"
            "memory.");
    exit(1);
  }   
  vm.ip = n;
...
void do_add(void)
{
  ...
  if (vm.sp != 0)
  {
    entry1 = vm.stack[vm.sp - 1];
  }
  do_pop();
  if (vm.sp != 0)
  {
    entry2 = vm.stack[vm.sp - 1];
  }
  do_pop();
  do_push(entry2 + entry1);
}
